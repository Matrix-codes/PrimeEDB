...
class FileManagerPage {
          ...
          <div class="table-container" style="flex:1; overflow-y:auto; border:none; border-radius:0;">
            <table>
              ...
              <tbody id="fm-file-list">
                <tr><td colspan="6" style="text-align:center; padding:32px; color:var(--text-muted);">Select a device to browse files</td></tr>
              </tbody>
            </table>
          </div>
  ...
  async _navigate(path, pushHistory = true) {
    ...
    const result = await window.primeADB.files.list(this.serial, path);
    if (!result.success) {
      toast.error('Cannot open directory', result.error);
  ...
  async _download(devicePath, name) {
    const result = await loading.wrap(() => window.primeADB.files.pull(this.serial, devicePath), `Downloading ${name}...`);
    result.success ? toast.success('Download complete', name) : toast.error('Download failed', result.error);
  }

  async _delete(devicePath, name) {
    const ok = await modal.confirm({ title: 'Delete File', message: `Delete <b>${name}</b>? This cannot be undone.`, confirmType: 'danger', confirmText: 'Delete' });
    if (!ok) return;
    const result = await loading.wrap(() => window.primeADB.files.delete(this.serial, devicePath), 'Deleting...');
    if (result.success) {
  ...
  async _upload() {
    ...
    const result = await loading.wrap(() => window.primeADB.files.push(this.serial, this.currentPath), 'Uploading...');
    if (result.success) {
  ...
  async _mkdir() {
    ...
    const result = await window.primeADB.files.mkdir(this.serial, newPath);
    result.success ? (toast.success('Folder created'), this._refresh()) : toast.error('Failed to create folder', result.error);
  }
