...
class LogcatPage {
  ...
  render(container) {
      ...
      <div class="page-enter" style="height:100%; display:flex; flex-direction:column; gap:0; overflow:hidden;">
        ...
        <div class="card" style="margin-bottom:var(--space-4); padding: var(--space-3) var(--space-4);">
          <div class="flex items-center gap-3" style="flex-wrap:wrap;">
            ...
            <input type="text" id="logcat-tag" placeholder="Filter by tag..." style="width:160px;" />
            <input type="text" id="logcat-search" placeholder="Search logs..." style="width:200px; flex:1;" />
            <div style="margin-left:auto; display:flex; gap:var(--space-2);">
              <button class="btn btn-secondary btn-sm" id="logcat-clear">🗑 Clear</button>
              <button class="btn btn-secondary btn-sm" id="logcat-export">💾 Export</button>
    ...
    window.primeADB.logcat.onData((line) => {
      this._addLog(line);
    });
  }
  ...
  async _start() {
    if (!this.serial) { toast.warning('Select a device first'); return; }
    if (this.isRunning) return;

    this.isRunning = true;
    this._updateUI();

    const result = await window.primeADB.logcat.start(this.serial, []);
    if (!result.success) {
      this.isRunning = false;
      this._updateUI();
      toast.error('Failed to start logcat', result.error);
    }
  }

  async _stop() {
    await window.primeADB.logcat.stop();
    this.isRunning = false;
    this._updateUI();
    toast.info('Logcat stopped');
  }
  ...
  async _export() {
    ...
    const result = await window.primeADB.logcat.export(content);
    result.success ? toast.success('Logcat exported!', result.filePath) : toast.error('Export failed', result.error);
  }
  ...
  destroy() {
    window.primeADB.logcat.stop();
    window.primeADB.logcat.offData();
    this.isRunning = false;
    this.logs = [];
  }
}
