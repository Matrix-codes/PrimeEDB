 ...
 */
function confirm({ title, message, confirmText, cancelText, confirmType = 'primary' }) {
  const cText = confirmText || (window.i18n ? window.i18n.t('common.confirm') : 'Confirm');
  const cancelT = cancelText || (window.i18n ? window.i18n.t('common.cancel') : 'Cancel');
    ...
    overlay.innerHTML = `
      <div class="modal animate-scale-in">
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" id="modal-cancel">${cancelT}</button>
          <button class="btn btn-${confirmType}" id="modal-confirm">${cText}</button>
        </div>
      </div>
    `;
 ...
 */
function prompt({ title, placeholder = '', defaultValue = '', confirmText }) {
  const cText = confirmText || (window.i18n ? window.i18n.t('common.ok') : 'OK');
  const cancelT = window.i18n ? window.i18n.t('common.cancel') : 'Cancel';
  return new Promise((resolve) => {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    overlay.innerHTML = `
      <div class="modal animate-scale-in">
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" id="modal-cancel">${cancelT}</button>
          <button class="btn btn-primary" id="modal-confirm">${cText}</button>
        </div>
      </div>
    `;
