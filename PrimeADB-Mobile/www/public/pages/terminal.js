    ...
    container.innerHTML = `
      <div class="page-enter" style="height:100%; display:flex; flex-direction:column; gap:0; overflow:hidden;">
        <div class="flex items-center gap-4" style="margin-bottom:var(--space-4); flex-wrap:wrap;">
          <div class="page-title" style="margin-bottom:0;"><span class="title-icon">💻</span> <span data-i18n="term.title">${window.i18n.t('term.title')}</span></div>
...
<span style="color:var(--text-muted);">Type a command below and press Enter to execute.</span>
          ...
          <div style="border-top:1px solid var(--color-border); padding:var(--space-2) var(--space-4);
            ...
            <input type="text" id="term-input"
                   ...
                   placeholder="${window.i18n.t('term.placeholder')}" data-i18n-placeholder="term.placeholder" autocomplete="off" spellcheck="false" />
  ...
  async _run() {
    ...
    this.isRunning = true;
    try {
      const result = await window.api.adb.shell(this.serial, cmd);
      this.isRunning = false;
      if (result.success) {
